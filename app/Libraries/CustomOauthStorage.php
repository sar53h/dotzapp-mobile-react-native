...

{
    ...
    public function __construct($connection, $config = array())
    {
        if (!$connection instanceof \PDO) {
            ...
            $connection = new \PDO($connection['dsn'], $connection['app_user_name'], $connection['password'], $connection['options']);
        }
        $this->db = $connection;
    ...
    public function getAccessToken($access_token)
    {
        ...
        if ($token = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            ...
            foreach ($AppUserRels as $appUserRel_key => $appUserRel) {
                if ($token['user_id'] === $appUserRel['app_user_id']) {
                    $profile = $modelProfiles->find( $appUserRel['profile_id'] );
                }
            }
